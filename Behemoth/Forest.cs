...
namespace Behemoth
{
    class Forest
    {
        ...
        public Forest(ObstacleList obstacles, Vector2 tLeft, int w, int h, int columns, int rows, Texture2D tree, Texture2D bush, Texture2D boulder)
        {
            timer = defaultTimer;
            treeSprite = tree;
            bushSprite = bush;
            ...
            for (int x = 0; x < columns-1; x += 2)
            {
                for (int y = 0; y < rows-1; y += 2)
                {
                    ...
                    if (obstacles.DidCollide(tempBush.HitBox) == null)
                    {
            ...
            for (int x = 0; x < columns-1; x += 3)
            {
                for (int y = 0; y < rows-1; y += 3)
                {
                    ...
                    if (obstacles.DidCollide(tempBoulder.HitBox) == null)
                    {
        ...
        public void Update(ObstacleList obstacles, GameTime gameTime, Player player, float camW, float camH)
        {
            var rnd = new Random(DateTime.Now.Millisecond);
            ...
            if (timer <= 0)
            {
                ...
                while(true)
                {
                    ...
                    if (obstacles.DidCollide(tempTree.HitBox) == null && IsOffScreen(tempTree.Position, player, camW, camH))
                    {
                        treesClose = obstacles.AdjacentObstacles(tempTree.HitPos).Count;
